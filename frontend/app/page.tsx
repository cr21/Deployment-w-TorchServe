  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(async () => {
      ...
      for (const jobId of pendingJobs) {
        ...
        const retryCount = retryMap.get(jobId) || 0;
        ...
        try {
          ...
          if (data.status === "SUCCESS") {
            ...
            // Increment retry count for PENDING status
            retryMap.set(jobId, retryCount + 1);
          }
        } catch (error) {
      ...
      if (completedJobs.length > 0) {
        ...
        completedJobs.forEach(jobId => retryMap.delete(jobId));
      }
    }, pollingInterval);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-background via-background/95 to-background/90 font-mono relative overflow-hidden">
      ...
      <main className="container mx-auto px-6 py-16 relative">
        {error && (
          <div className="max-w-4xl mx-auto mb-4 p-4 bg-red-500/10 text-red-500 rounded-lg">
            {error}
          </div>
        )}
